...
At each time step, the model is trained only on data available up to that point and is required to forecast the next observation. This one-step-ahead setup exposes how the model reacts to new information, making the effects of lag, overshoot, and stability directly observable. Multi-step forecasts are also examined to understand how errors propagate when predictions depend increasingly on prior forecasts rather than observed data.This evaluation approach is chosen because it highlights adaptation dynamics and component interactions, which are obscured when models are evaluated solely through retrospective curve fitting. Accuracy metrics such as RMSE and MAE are used only as secondary checks, not as decision drivers. The emphasis remains on understanding how the model behaves under incremental information arrival and how its assumptions influence forecast evolution over time.

When evaluated using walk-forward (expanding window) validation, Holt–Winters exhibits stable and coherent forecasting behavior on the AirPassengers dataset. One-step-ahead forecasts consistently achieve lower RMSE and MAE compared to multi-step forecasts, reflecting the model’s reliance on recently updated states rather than extrapolated dynamics.
Given the 12-month seasonal period, the model requires at least two full seasonal cycles (approximately 24 months) before forecasts stabilize. Once this minimum history is available, seasonal indices and trend estimates become sufficiently well-formed, leading to a noticeable improvement and subsequent stabilization in error metrics.
Beyond this point, additional training data yields diminishing improvements in short-horizon accuracy, suggesting that the core seasonal and trend structure has already been captured. This behavior confirms that Holt–Winters is structurally well-aligned with the data and behaves consistently under time-respecting validation.
